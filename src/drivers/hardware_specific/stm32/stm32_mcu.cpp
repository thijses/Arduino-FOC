...
void _stopTimers(HardwareTimer **timers_to_stop, int timer_num)
{
  ...
  int _getInternalSourceTrigger(HardwareTimer* master, HardwareTimer* slave) {
    ...
    #if defined(TIM1) && defined(LL_TIM_TS_ITR0)
      if(TIM_master == TIM1) {
        if((TIM_slave == TIM2) || (TIM_slave == TIM3) || (TIM_slave == TIM4)) { return(LL_TIM_TS_ITR0); } // TLD order of operations check
        #if defined(TIM8)
        ...
        if((TIM_slave == TIM1) || (TIM_slave == TIM3) || (TIM_slave == TIM4)) { return(LL_TIM_TS_ITR1); } // TLD order of operations check
        #if defined(TIM8)
        ...
        if((TIM_slave== TIM1) || (TIM_slave == TIM2) || (TIM_slave == TIM4)) { return(LL_TIM_TS_ITR2); } // TLD order of operations check
        #if defined(TIM5)
        ...
        if((TIM_slave == TIM1) || (TIM_slave == TIM2) || (TIM_slave == TIM3)) { return(LL_TIM_TS_ITR3); } // TLD order of operations check
        #if defined(TIM8)
        ...
        else if((TIM_slave ==TIM4) || (TIM_slave ==TIM5)) { return(LL_TIM_TS_ITR3); } // TLD order of operations check
      }
    #endif
...
STM32DriverParams* _initHardware6PWMPair(long PWM_freq, float dead_zone, PinMap* pinH, PinMap* pinL, STM32DriverParams* params, int paramsPos) {
  // sanity check
  if((pinH==NP) || (pinL==NP)) { // TLD order of operations check
  ...
  // more sanity check
  if((channel1!=channel2) || (pinH->peripheral!=pinL->peripheral)) { // TLD order of operations check
    return((STM32DriverParams*)SIMPLEFOC_DRIVER_INIT_FAILED);
  }
  ...
  if((dead_time==0) && (dead_zone>0)) { // TLD order of operations check
...
int scoreCombination(int numPins, PinMap* pinTimers[]) {
  ...
  for(int i=0; i<numTimerPinsUsed; i++) {
    if((pinTimers[i]->peripheral == timerPinsUsed[i]->peripheral)
        && (STM_PIN_CHANNEL(pinTimers[i]->function) == STM_PIN_CHANNEL(timerPinsUsed[i]->function))) { // TLD order of operations check
  ...
  for(int i=0; i<numPins-1; i++) {
    for(int j=i+1; j<numPins; j++) {
      if((pinTimers[i]->peripheral == pinTimers[j]->peripheral)
          ...
          && (STM_PIN_INVERTED(pinTimers[i]->function) == STM_PIN_INVERTED(pinTimers[j]->function))) { // TLD order of operations check
  ...
  if(numPins==6) {
    ...
    if((pinTimers[0]->peripheral == pinTimers[1]->peripheral)
        ...
        && STM_PIN_INVERTED(pinTimers[1]->function) && STM_PIN_INVERTED(pinTimers[3]->function) && STM_PIN_INVERTED(pinTimers[5]->function)) { // TLD order of operations check
      ...
      // TODO check these defines
        ...
        if((STM_PIN_CHANNEL(pinTimers[0]->function)>3) || (STM_PIN_CHANNEL(pinTimers[2]->function)>3) || (STM_PIN_CHANNEL(pinTimers[4]->function)>3) ) { // TLD order of operations check
          return(-8); // channel 4 does not have dead-time insertion
        }
      #endif
      #ifdef STM32G4xx_HAL_TIM_H
        if((STM_PIN_CHANNEL(pinTimers[0]->function)>4) || (STM_PIN_CHANNEL(pinTimers[2]->function)>4) || (STM_PIN_CHANNEL(pinTimers[4]->function)>4) ) { // TLD order of operations check
      ...
      if((pinTimers[0]->peripheral != pinTimers[1]->peripheral)
        || (pinTimers[2]->peripheral != pinTimers[3]->peripheral)
        || (pinTimers[4]->peripheral != pinTimers[5]->peripheral)) { // TLD order of operations check
...
int findIndexOfLastPinMapEntry(int pin) {
  ...
  while(PinMap_TIM[i].pin!=NC) {
    if(   (pinName == (PinMap_TIM[i].pin & ~ALTX_MASK)) 
        && (pinName != (PinMap_TIM[i+1].pin & ~ALTX_MASK))) { // TLD order of operations check
      return(i);
    }
    i++;
  }
...
int findBestTimerCombination(int numPins, int index, int pins[], PinMap* pinTimers[]) {
  ...
  if((startIndex == -1) || (endIndex == -1)) { // TLD order of operations check
  ...
  for(int i=startIndex;i<=endIndex;i++) {
    ...
    int score = NOT_FOUND;
    if(index<(numPins-1)) { // TLD order of operations check
    ...
    if((score>=0) && (score<bestScore)) { // TLD order of operations check
      bestScore = score;
...
void _writeDutyCycle6PWM(float dc_a, float dc_b, float dc_c, PhaseState* phase_state, void* params) {
  switch(((STM32DriverParams*)params)->interface_type) {
    ...
    case(_SOFTWARE_6PWM):
      ...
      if((phase_state[0] == PhaseState::PHASE_ON) || (phase_state[0] == PhaseState::PHASE_HI)) { // TLD order of operations check
      ...
      if((phase_state[0] == PhaseState::PHASE_ON) || (phase_state[0] == PhaseState::PHASE_LO)) { // TLD order of operations check
      ...
      if((phase_state[1] == PhaseState::PHASE_ON) || (phase_state[1] == PhaseState::PHASE_HI)) { // TLD order of operations check
      ...
      if((phase_state[1] == PhaseState::PHASE_ON) || (phase_state[1] == PhaseState::PHASE_LO)) { // TLD order of operations check
      ...
      if((phase_state[2] == PhaseState::PHASE_ON) || (phase_state[2] == PhaseState::PHASE_HI)) { // TLD order of operations check
      ...
      if((phase_state[2] == PhaseState::PHASE_ON) || (phase_state[2] == PhaseState::PHASE_LO)) { // TLD order of operations check
