...
struct wo_association& getWOAssociation(EPortType port, uint32_t pin) {
  for(int i=0;i<NUM_WO_ASSOCIATIONS;i++) {
    if((WO_associations[i].port==port) && (WO_associations[i].pin==pin)) { // TLD order of operations check
...
EPioType getPeripheralOfPermutation(int permutation, int pin_position) {
  return((((permutation>>pin_position)&0x01)==0x1)?PIO_TIMER_ALT:PIO_TIMER); // TLD order of operations check
}
