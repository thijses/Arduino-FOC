...
void setupPWM(int pin_nr, long pwm_frequency, bool invert, RP2040DriverParams* params, uint8_t index) {
  ...
  uint32_t factor = (4096 * 2) * pwm_frequency; // TLD order of operations check
  uint32_t div = sysclock_hz / factor;
  if((sysclock_hz % factor) !=0) { div+=1; } // TLD order of operations check
  if(div < 16) { div = 16; }
  uint32_t wrapvalue = (((sysclock_hz * 8) / div) / pwm_frequency) - 1; // TLD order of operations check
...
void* _configure1PWM(long pwm_frequency, const int pinA) {
  ...
  if( (!pwm_frequency) || (!_isset(pwm_frequency)) ) { pwm_frequency = _PWM_FREQUENCY; } // TLD order of operations check
