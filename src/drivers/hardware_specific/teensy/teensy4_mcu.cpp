...
IMXRT_FLEXPWM_t* get_flexpwm(uint8_t pin) {
  ...
  if((pin >= CORE_NUM_DIGITAL) || (info->type == 2)) { // TLD order of operations check
...
void setup_pwm_pair (IMXRT_FLEXPWM_t * flexpwm, int submodule, bool ext_sync,  const long frequency, float dead_zone )
{
  ...
  uint32_t newdiv = (uint32_t)((((float)F_BUS_ACTUAL) / frequency) + 0.5f); // TLD order of operations check
  uint32_t prescale = 0;
  //printf(" div=%lu\n", newdiv);
...
void write_pwm_pair(IMXRT_FLEXPWM_t * flexpwm, int submodule, float duty) {
  ...
  int mid_pwm = int((half_cycle)/2.0f);
  int count_pwm = int(mid_pwm*((duty*2)-1)) + mid_pwm; // TLD order of operations check
...
#endif // SIMPLEFOC_TEENSY4_FORCE_CENTER_ALIGNED_3PWM
