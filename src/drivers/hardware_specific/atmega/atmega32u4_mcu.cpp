...
void _pinHighFrequency(const int pin) {
  ...
  if((pin == 3) || (pin == 11) ) { // TLD order of operations check
  ...
  } else if( (pin == 6) || (pin == 13) ) {  // a bit more complicated 10 bit timer // TLD order of operations check
    // PLL Configuration
...
int _configureComplementaryPair(int pinH, int pinL) {
  if( ((pinH == 3) && (pinL == 11) ) || ((pinH == 11) && (pinL == 3) ) ) { // TLD order of operations check
  ...
  } else if(((pinH == 6) && (pinL == 13) ) || ((pinH == 13) && (pinL == 6) ) ) { // TLD order of operations check
    // PLL Configuration
    PLLFRQ= ((PLLFRQ & 0b11001111) | 0x20); // Use 96MHz / 1.5 = 64MHz
    TCCR4B = ((TCCR4B & 0b11110000) | 0xB); // configure prescaler to get 64M/2/1024 = 31.25 kHz
    TCCR4D = ((TCCR4D & 0b11111100) | 0x01); // configure the pwm phase-corrected mode
