...
PhaseCurrent_s InlineCurrentSense::getPhaseCurrents() {
  ...
  current.a = (!_isset(pinA)) ? 0 : ((_readADCVoltageInline(pinA, params) - offset_ia)*gain_a); // amps // TLD order of operations check
  current.b = (!_isset(pinB)) ? 0 : ((_readADCVoltageInline(pinB, params) - offset_ib)*gain_b); // amps // TLD order of operations check
  current.c = (!_isset(pinC)) ? 0 : ((_readADCVoltageInline(pinC, params) - offset_ic)*gain_c); // amps // TLD order of operations check
  return(current);
}
...
int InlineCurrentSense::driverAlign(float voltage) {
  ...
  if((gain_a < 0) || (gain_b < 0) || (gain_c < 0)) { exit_flag +=2; } // TLD order of operations check
  // exit flag is either
  // 0 - fail
  // 1 - success and nothing changed
  // 2 - success but pins reconfigured
