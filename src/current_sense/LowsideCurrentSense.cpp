...
PhaseCurrent_s LowsideCurrentSense::getPhaseCurrents() {
  ...
  _startADC3PinConversionLowSide();
  current.a = (!_isset(pinA)) ? 0 : ((_readADCVoltageLowSide(pinA, params) - offset_ia)*gain_a); // amps // TLD order of operations check
  current.b = (!_isset(pinB)) ? 0 : ((_readADCVoltageLowSide(pinB, params) - offset_ib)*gain_b); // amps // TLD order of operations check
  current.c = (!_isset(pinC)) ? 0 : ((_readADCVoltageLowSide(pinC, params) - offset_ic)*gain_c); // amps // TLD order of operations check
  return(current);
}
...
int LowsideCurrentSense::driverAlign(float voltage) {
  ...
  if(_isset(pinA)) {
    ...
    if(_isset(pinB) && (ab_ratio > 1.5f) ) { // should be ~2 // TLD order of operations check
      gain_a *= _sign(c.a);
    } else if(_isset(pinC) && (ac_ratio > 1.5f) ) { // should be ~2 // TLD order of operations check
      gain_a *= _sign(c.a);
    } else if(_isset(pinB) && (ab_ratio < 0.7f) ) { // should be ~0.5 // TLD order of operations check
    ...
    } else if(_isset(pinC) &&  (ac_ratio < 0.7f) ) { // should be ~0.5 // TLD order of operations check
  ...
  if(_isset(pinB)) {
    ...
    if(_isset(pinA) && (ba_ratio > 1.5f) ) { // should be ~2 // TLD order of operations check
      gain_b *= _sign(c.b);
    } else if(_isset(pinC) && (bc_ratio > 1.5f) ) { // should be ~2 // TLD order of operations check
      gain_b *= _sign(c.b);
    } else if(_isset(pinA) && (ba_ratio < 0.7f) ) { // it should be ~0.5 // TLD order of operations check
      // switch phase A and B
      int tmp_pinB = pinB;
      pinB = pinA;
      pinA = tmp_pinB;
      ...
      gain_b *= _sign(c.a);
      exit_flag = 2; // signal that pins have been switched
    } else if(_isset(pinC) && (bc_ratio < 0.7f) ) { // should be ~0.5 // TLD order of operations check
      // switch phase A and C
      int tmp_pinB = pinB;
      pinB = pinC;
      pinC = tmp_pinB;
  ...
  if(_isset(pinC)) {
    ...
    if(_isset(pinA) && (ca_ratio > 1.5f) ) { // should be ~2 // TLD order of operations check
      gain_c *= _sign(c.c);
    } else if(_isset(pinB) && (cb_ratio > 1.5f) ) { // should be ~2 // TLD order of operations check
      gain_c *= _sign(c.c);
    } else if(_isset(pinA) && (ca_ratio < 0.7f) ) { // it should be ~0.5 // TLD order of operations check
      // switch phase A and C
      int tmp_pinC = pinC;
      pinC = pinA;
      pinA = tmp_pinC;
      ...
      gain_c *= _sign(c.a);
      exit_flag = 2; // signal that pins have been switched
    } else if(_isset(pinB) && (cb_ratio < 0.7f) ) { // should be ~0.5 // TLD order of operations check
