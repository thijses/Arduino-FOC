...
float _readADCVoltageInline(const int pinA, const void* cs_params) {
  ...
  if((pinA>=26) && (pinA<=29) && engine.channelsEnabled[pinA-26]) { // TLD order of operations check
...
bool RP2040ADCEngine::init() {
  ...
  if((samples_per_second<1) || (samples_per_second>=500000)) { // TLD order of operations check
