...
void Commander::motor(FOCMotor* motor, char* user_command) {
  ...
  // if target setting
  if(isDigit(user_command[0]) || (user_command[0] == '-') || (user_command[0] == '+') || isSentinel(user_command[0])) { // TLD order of operations check
  ...
  int value_index = ((sub_cmd >= 'A'  && sub_cmd <= 'Z') ||  (sub_cmd == '#')) ?  2 :  1; // TLD order of operations check
  // check if get command
  bool GET = isSentinel(user_command[value_index]);
  // parse command values
  float value = atof(&user_command[value_index]);
  printMachineReadable(cmd);
  if((sub_cmd >= 'A')  && (sub_cmd <= 'Z')) { // TLD order of operations check
      ...
      switch(sub_cmd) {
        case(SCMD_LIM_VOLT):      // voltage limit change
          ...
          if(!GET) {
            ...
            if( (!_isset(motor->phase_resistance)) && (motor->torque_controller==TorqueControlType::voltage)) { motor->PID_velocity.limit = value; } // TLD order of operations check
          }
  ...
  switch(cmd) {
    case(CMD_MOTION_TYPE):
      ...
      switch(sub_cmd) {
        ...
        default:
          // change control type
          if((!GET) && (value >= 0) && ((int)value < 5)) { // if set command // TLD order of operations check
    ...
    case(CMD_TORQUE_TYPE):
      ...
      if((!GET) && ((int8_t)value >= 0) && ((int8_t)value < 3))// if set command // TLD order of operations check
  ...
  switch(motor->controller) {
    ...
    case(MotionControlType::velocity): // setting velocity target + torque limit
      ...
      if(next_value) {
        ...
        if((!_isset(motor->phase_resistance)) && (motor->torque_controller == TorqueControlType::voltage)) { motor->voltage_limit = torque; } // TLD order of operations check
...
void Commander::print(const int number) {
  if( (!com_port) || (verbose == VerboseMode::nothing) ) { return; } // TLD order of operations check
